...
func (e CategoryEdges) ProductsOrErr() ([]*Products, error) {
	if e.loadedTypes[0] {
...
func (*Category) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case category.FieldID, category.FieldCreatedBy, category.FieldUpdatedBy, category.FieldDeletedBy, category.FieldCategoryID:
	...
	for i := range columns {
		...
		case category.FieldDeletedBy:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				c.DeletedBy = new(int)
				*c.DeletedBy = int(value.Int64)
			}
		case category.FieldCategoryID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field category_id", values[i])
			} else if value.Valid {
				c.CategoryID = new(int)
...
func (c *Category) QueryProducts() *ProductsQuery {
