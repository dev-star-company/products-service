...
func (cq *CategoryQuery) QueryProducts() *ProductsQuery {
	query := (&ProductsClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) WithProducts(opts ...func(*ProductsQuery)) *CategoryQuery {
	query := (&ProductsClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withProducts; query != nil {
		if err := cq.loadProducts(ctx, query, nodes,
			func(n *Category) { n.Edges.Products = []*Products{} },
			func(n *Category, e *Products) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CategoryQuery) loadProducts(ctx context.Context, query *ProductsQuery, nodes []*Category, init func(*Category), assign func(*Category, *Products)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(products.FieldCategoryID)
	}
