...
func (uc *UserCreate) AddProducts(p ...*Products) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
...
func (uc *UserCreate) AddCreatedProductIDs(ids ...int) *UserCreate {
	uc.mutation.AddCreatedProductIDs(ids...)
	return uc
}

// AddCreatedProducts adds the "created_products" edges to the Products entity.
func (uc *UserCreate) AddCreatedProducts(p ...*Products) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddCreatedProductIDs(ids...)
}

// AddUpdatedProductIDs adds the "updated_products" edge to the Products entity by IDs.
func (uc *UserCreate) AddUpdatedProductIDs(ids ...int) *UserCreate {
	uc.mutation.AddUpdatedProductIDs(ids...)
	return uc
}

// AddUpdatedProducts adds the "updated_products" edges to the Products entity.
func (uc *UserCreate) AddUpdatedProducts(p ...*Products) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddUpdatedProductIDs(ids...)
}

// AddDeletedProductIDs adds the "deleted_products" edge to the Products entity by IDs.
func (uc *UserCreate) AddDeletedProductIDs(ids ...int) *UserCreate {
	uc.mutation.AddDeletedProductIDs(ids...)
	return uc
}

// AddDeletedProducts adds the "deleted_products" edges to the Products entity.
func (uc *UserCreate) AddDeletedProducts(p ...*Products) *UserCreate {
	ids := make([]int, len(p))
	for i := range p {
		ids[i] = p[i].ID
	}
	return uc.AddDeletedProductIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.CreatedProductsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.UpdatedProductsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
	...
	if nodes := uc.mutation.DeletedProductsIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
