...
func (bq *BrandQuery) QueryProducts() *ProductsQuery {
	query := (&ProductsClient{config: bq.config}).Query()
...
func (bq *BrandQuery) WithProducts(opts ...func(*ProductsQuery)) *BrandQuery {
	query := (&ProductsClient{config: bq.config}).Query()
...
func (bq *BrandQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Brand, error) {
	...
	if query := bq.withProducts; query != nil {
		if err := bq.loadProducts(ctx, query, nodes,
			func(n *Brand) { n.Edges.Products = []*Products{} },
			func(n *Brand, e *Products) { n.Edges.Products = append(n.Edges.Products, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (bq *BrandQuery) loadProducts(ctx context.Context, query *ProductsQuery, nodes []*Brand, init func(*Brand), assign func(*Brand, *Products)) error {
